...
| Event Trace Firmware Design | [link](../development/FirmwareDesign/event_tracing.md) |
...
4. Define your provider in your new header. Ex:
    ...
    **NOTE**: The `FPFW_ET_DEFINE_PROVIDER_EX` will generate metadata used associate events with providers and the current provider logging level. Metadata will either be stored in flash or TCM.
...
1. Add an event to the same header you defined your provider in. Ex:
    ...
    **NOTE**: The `FPFW_ET_DEFINE_EVENT` will generate metadata used to parse the event AND an `EventWrite<EventName>` function to fill in the event to the event trace buffering system. Metadata will either be stored in flash or TCM.
