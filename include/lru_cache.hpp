...
//
// Locking strategy: a single std::mutex guards all operations.
//
// Why not shared_mutex with shared_lock on get()?
// get() calls list_.splice(), which physically relinks list nodes — it mutates
// internal state. Allowing multiple threads to hold a shared lock while each
// calls splice() is a data race on the list. Therefore get() must also hold
// an exclusive lock.
