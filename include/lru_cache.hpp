...
// Note: get() is NOT a read-only operation — it splices the accessed node to
// the front of the list (MRU promotion), so it mutates internal state.
// Therefore both get() and put() must hold an exclusive lock.
...
public:
    ...
    void put(const K& key, V value) {
        ...
        // Evict LRU entries while at or over capacity before inserting.
