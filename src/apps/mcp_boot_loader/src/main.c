...
uint8_t mainImage[MCP_MAX_IMAGE_SIZE] __attribute__((section(".mainimage"))) = {0};
...
kingsgate_boot_config_t boot_config = {
    ...
    .itc_ram_base = (size_t)MCP_ITCM_RAM_BASE,
    ...
    .cpu_type = MSCP_CPU_MCP,
};

//! Will be overridden by the MCP firmware
...
/**
 *  @brief This function is main function of MCP boot loader.
 *
...
int main(void)
{
    ...
    arch_init_shcsr();
    //! Set the initial MPU settings for bootloader which will be overridden by the MCP firmware
    mpu_init(region, 1);
