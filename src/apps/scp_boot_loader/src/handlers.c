...
#else
    #include <scp_mmap.h> // for SCP_DTCM_RAM_SIZE, SCP_MSCP_EXP_SRAM0_ADDR,SCP_TOP_SCP_DATA_RAM_ADDRESS, SCP_TOP_SC...
#endif
...
NORETURN void arch_exception_reset(void)
{
    ...
    {
        for (uint32_t mem_offset = 0; mem_offset < (uint32_t)SCP_TOP_SCP_DATA_RAM_SIZE; mem_offset += sizeof(uint32_t))
        {
