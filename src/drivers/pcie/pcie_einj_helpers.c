...
silibs_status_t handle_pcie_error_injection(PDFWK_SYNC_REQUEST_HEADER req)
{
    ...
    case PCIE_ERROR_TYPE_RP_INTERNAL_CRC:
        sts = pcie_rp_rasdes_inject_crc_error(&(rpss->rps[rp_index]), pcie_params->error_data.crc, op->error_count);
        break;
    case PCIE_ERROR_TYPE_RP_INTERNAL_SEQNUM:
        sts = pcie_rp_rasdes_inject_seqnum_error(&(rpss->rps[rp_index]), pcie_params->error_data.seqnum, op->error_count);
        break;
    case PCIE_ERROR_TYPE_RP_INTERNAL_DLLP:
        sts = pcie_rp_rasdes_inject_dllp_error(&(rpss->rps[rp_index]), pcie_params->error_data.dllp, op->error_count);
        break;
    case PCIE_ERROR_TYPE_RP_INTERNAL_SYMBOL:
        sts = pcie_rp_rasdes_inject_symbol_error(&(rpss->rps[rp_index]), pcie_params->error_data.symbol, op->error_count);
        break;
    case PCIE_ERROR_TYPE_RP_INTERNAL_FC:
        sts = pcie_rp_rasdes_inject_fc_error(&(rpss->rps[rp_index]), pcie_params->error_data.fc, op->error_count);
        break;
    case PCIE_ERROR_TYPE_RP_INTERNAL_RETRY_TLP:
        sts = pcie_rp_rasdes_inject_retry_tlp_error(&(rpss->rps[rp_index]), pcie_params->error_data.retry_tlp, op->error_count);
        break;
    case PCIE_ERROR_TYPE_RP_INTERNAL_PHY_SET_SRAM_ERROR:
        sts = pcie_phy_setup_error_injection(rpss->rps[rp_index].bases._base,
    ...
    case PCIE_ERROR_TYPE_RP_INTERNAL_PHY_CLEAR_SRAM_ERROR:
        sts = pcie_phy_clear_injection(rpss->rps[rp_index].bases._base, pcie_params->error_data.phy_sram.phy_inst);
        break;
    default:
        sts = SILIBS_E_PARAM;
        goto done;
