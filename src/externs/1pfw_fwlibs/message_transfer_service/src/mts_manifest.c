...
void mts_create_manifest_from_elf(uintptr_t atu_base_addr, size_t max_size)
{
...
void mts_build_diag_decoder_full_manifest(void)
{
    ...
    {
        printf("MTS: SCP Manifest Parser Type is not Packed\n");
        FPFW_ET_LOG(MtsManifestCreateFail, MTS_PLATFORM_CORE_SCP);
        return;
    }
    ...
    mts_create_manifest_from_elf((uintptr_t)mcp_dest_manifest_hdr, remaining_dest_size);
