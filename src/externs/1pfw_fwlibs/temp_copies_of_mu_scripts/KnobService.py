...
def generate_public_header(schema, header_path, efi_type=False, types_only=False, no_change=False):
    ...
    format_options.c_no_mangle = no_change
...
def generate_profiles(schema, profile_header_path, profile_paths, efi_type, profile_names=None, profile_ids=None, no_change=False):
...
def generate_sources(schema, public_header, service_header, data_header, efi_type, options):
    ...
    if efi_type is True:
        ...
        if data_header:
            generate_cached_implementation(schema, data_header, efi_type, options.no_change)
    elif service_header:
        generate_cached_implementation(schema, service_header, efi_type, options.no_change)
    ...
    if sys.argv[arg_num].lower() == "generateheader" or sys.argv[arg_num].lower() == "generateheader_efi":
        ...
        if (len(sys.argv) >= arg_num + 1) and not known_args.types_only:
            ...
            generate_profiles(schema, profile_header_path, profile_paths, efi_type,
                              profile_names=known_args.profile_names, profile_ids=formatted_profile_ids, no_change=known_args.no_change)
        return 0
