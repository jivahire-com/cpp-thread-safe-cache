...
uint32_t crash_dump_get_dump_size(uint8_t die_id, crash_dump_core_t core_id)
{
    (void)die_id;
    (void)(core_id);
    // ToDo:Need use the die_id and core_id to get the real ddr mapping address
    DUMP_HEADER header = {};
    crash_dump_context_t* ctx = crash_dump_context();
    crash_dump_type_context_t* type_context =
        ctx->type_ctx[CRASH_DUMP_TYPE_FULL]; // Accelerator crash dump is only for full dump.
    ...
    FPFwCDMemcpyGlobalToLocal(&type_context->crash_dump_ctx.memAPIs, &header, CRASH_DUMP_FULL_MCP_ADDR, sizeof(DUMP_HEADER));
...
bool crash_dump_is_dump_complete(uint8_t die_id, crash_dump_core_t core_id)
{
    ...
    {
        FPFwCDMemcpyGlobalToLocal(&ctx->type_ctx[CRASH_DUMP_TYPE_FULL]->crash_dump_ctx.memAPIs,
                                  &header,
                                  CRASH_DUMP_FULL_MCP_ADDR,
                                  sizeof(DUMP_HEADER));

// Ignore multichar constant warning (defined by shared lib)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wmultichar"
        return header.Magic == DUMP_HEADER_MAGIC_COMPLETE;
