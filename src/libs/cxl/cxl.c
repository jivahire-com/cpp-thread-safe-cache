...
{
    cxl_region_params_t cxl_params[NUM_CXL_PARAMS] = {config_get_cxl_params_die0(), config_get_cxl_params_die1()};
    ...
    for (int i = 0; i < NUM_CXL_PARAMS; i++)
    {
        ...
        if (cxl_params[i].interleave_ways != INTERLEAVE_NONE)
        {
            ...
            printf("CHBCR: Adding decoder with interleave size %d, ways %d\n",
                   cxl_params[i].interleave_size,
                   cxl_params[i].interleave_ways);

            uint64_t cxl_region_size =
                set_cxl_mem_region_base_and_size(cxl_region, &cxl_region_base, cxl_params[i].interleave_ways);
            printf("CHBCR: Decoder with base 0x%lX%08lX size 0x%lX%08lX\n",
                   (unsigned long)(cxl_region_base >> 32),
                   (unsigned long)cxl_region_base,
                   (unsigned long)(cxl_region_size >> 32),
                   (unsigned long)(cxl_region_size));
            ...
            for (int port_index = 0; port_index < (int)cxl_params[i].interleave_ways; port_index++)
            {
                ...
                cxl_region->target_list.target_port_id[port_index] =
                    get_rp_port_number_from_cxl_port(cxl_params[i].ports[port_index]);
        ...
        else
        {
            ...
            for (int port_index = 0;
                 (cxl_params[i].ports[port_index] != CXL_PORT_EMPTY) && (port_index < (int)INTERLEAVE_4_WAYS);
                 port_index++)
            {
                cxl_hdm_decoder_region_t* cxl_region = (cxl_hdm_decoder_region_t*)target_address;
                memset(cxl_region, 0, sizeof(cxl_hdm_decoder_region_t));
