...
typedef struct
{
    uint16_t vmin_mV[NUM_AP_CORES_PER_DIE];
    uint8_t sequence_number; // 0-255, increments on each write, rolls over
} pwr_tlm_core_exch_vmin_t;
...
{
    memcpy(s_pwr_tlm_core_exch->scp_to_mcp_24hr.vmin.vmin_mV,
           vmin_array,
           sizeof(s_pwr_tlm_core_exch->scp_to_mcp_24hr.vmin.vmin_mV));
    s_pwr_tlm_core_exch->scp_to_mcp_24hr.vmin.sequence_number++;

    // Clean (flush) the data cache line
    SCB_CleanDCache_by_Addr(&s_pwr_tlm_core_exch->scp_to_mcp_24hr.vmin, sizeof(pwr_tlm_core_exch_vmin_t));
}

uint8_t pwr_tlm_core_exch_mcp_read_vmin(uint16_t* vmin_array)
{
    // Invalidate the data cache line
    SCB_InvalidateDCache_by_Addr(&s_pwr_tlm_core_exch->scp_to_mcp_24hr.vmin, sizeof(pwr_tlm_core_exch_vmin_t));

    memcpy(vmin_array,
           s_pwr_tlm_core_exch->scp_to_mcp_24hr.vmin.vmin_mV,
           sizeof(s_pwr_tlm_core_exch->scp_to_mcp_24hr.vmin.vmin_mV));

    return s_pwr_tlm_core_exch->scp_to_mcp_24hr.vmin.sequence_number;
}
