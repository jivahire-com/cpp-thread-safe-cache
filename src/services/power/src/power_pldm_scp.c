...
{
    POWER_LOG_INFO("[PLDM POWER CAP UPDATE] Result %d Current %d W Previous %d W\n",
                   result,
                   current_cap,
...
static void pwr_pldm_icc_recv_cb(void* context, size_t output_size_bytes, fpfw_status_t status)
{
    ...
    case ICC_COMMAND_PLDM_PWR_CAP:
        ...
        uint16_t current_power_cap = power_cap_is_capped() ? get_current_soc_power_cap() : pwr_cap_upper_limit;
        POWER_LOG_INFO("[PWR PLDM] Received Power Cap Request: Requested %d W, Current %d W\n", requested_pwr_cap, current_power_cap);
        ...
        //! 1. Verify if the power cap is within limits
        if (requested_pwr_cap < POWER_CAP_MIN || requested_pwr_cap > pwr_cap_upper_limit)
        {
            POWER_LOG_ERR("[PWR PLDM] Power cap out of range, Expected Range: [%d, %d], Received: %d!",
                          POWER_CAP_MIN,
                          pwr_cap_upper_limit,
                          requested_pwr_cap);
            power_cap_completed_callback(MP_POWER_CAP_FAIL_INVALID_REQUEST, requested_pwr_cap, current_power_cap);
            break;
        }
        ...
        if (current_power_cap != requested_pwr_cap)
        {
            ...
            else
            {
                ...
                POWER_LOG_INFO("[PWR PLDM] Power cap update pending, new cap %d\n", requested_pwr_cap);
            }
        }
        else
        {
