...
void data_proc_tlm_cmpnt_get_pwr_core_pstate_data(uint16_t core_id, pwr_core_event_pstate_t (*pstate_array)[NUMBER_OF_PSTATES])
{
    ...
    for (uint16_t pstate = 0; pstate < NUMBER_OF_PSTATES; pstate++)
    {
        (*pstate_array)[pstate].pstate_id = pstate;
    }
}
...
{
    FPFW_UNUSED(core_id);
    FPFW_UNUSED(rack_priority_array);
}
