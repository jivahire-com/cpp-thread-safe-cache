...
} power_telemetry_event_t; // WAS POWER_TELEMETRY_EVENT_T
...
} performance_telemetry_event_t; // WAS PERFORMANCE_TELEMETRY_EVENT_T


// TODO: rename report type
typedef enum event_category_t
{
    EVENT_CATEGORY_POWER = 0,
    EVENT_CATEGORY_PERF,
    MAX_EVENT_CATEGORY
} event_category_t;
...
typedef struct {
    //union {
        //TODO: definition will come from DCS
       // encoder_header_t encoder_info;
       // decoder_header_t decoder_info;
    //};
    union {
        telemetry_client_header_t client_header;
    };
} telemetry_package_hdr_t;


typedef struct {
    uint32_t event_schema_id;   // WAS data_schema_id;
    uint16_t collection_id;     // WAS component_id
    uint16_t number_of_events;  // WAS element_count
    uint32_t collection_size;   // minus the collection header size
} telemetry_collection_hdr_t;   // WAS telemetry_component_t

typedef struct {
    uint64_t timestamp;
    uint32_t record_number;
    uint32_t number_of_collections;  // WAS component_count;
    uint32_t record_size;  //  minus the record header size
} telemetry_record_hdr_t;
...
} pwr_soc_collection_pc3_t;

typedef struct {
    telemetry_record_hdr_t record_header;
    pwr_soc_collection_pc3_t pc3_collection;
} pwr_soc_record_pc3_t;
...
} pwr_soc_event_rail_t;

typedef struct {
    telemetry_collection_hdr_t collection_header;
    pwr_soc_event_rail_t rail_event;
} pwr_soc_collection_vrail_t;

typedef struct {
    telemetry_record_hdr_t record_header;
    pwr_soc_collection_vrail_t rail_collection[MAX_NUM_OF_VR_RAILS];
} pwr_soc_record_vrail_t;

//----------------POWER_TELEMETRY_EVENT_SOC_HNF----------------
typedef temperature_t pwr_soc_event_hnf_t;

typedef struct {
    telemetry_collection_hdr_t collection_header;
    pwr_soc_event_hnf_t hnf_event;
} pwr_soc_collection_hnf_t;

typedef struct {
    telemetry_record_hdr_t record_header;
    pwr_soc_collection_hnf_t hnf_collection[NUMBER_OF_HNF_CHANNELS_PER_DIE];
} pwr_soc_record_hnf_t;
...
typedef struct {
    temperature_t s0;
    temperature_t s1;
} pwr_soc_event_dimm_t;

typedef struct {
    telemetry_collection_hdr_t collection_header;
    pwr_soc_event_dimm_t dimm_event;
} pwr_soc_collection_dimm_t;

typedef struct {
    telemetry_record_hdr_t record_header;
    pwr_soc_collection_dimm_t entry[NUMBER_OF_DIMM_MODULES];
} pwr_soc_record_dimm_t;

//----------------POWER_TELEMETRY_EVENT_SOC_SENSOR_TEMP----------------

typedef temperature_t pwr_soc_event_sensor_temp_t;

typedef struct {
    telemetry_collection_hdr_t collection_header;
    pwr_soc_event_sensor_temp_t sensor_temp_event;
} pwr_soc_collection_sensor_temp_t;

typedef struct {
    telemetry_record_hdr_t record_header;
    pwr_soc_event_sensor_temp_t sensor_temp_collection[NUMBER_OF_SOC_TEMP_SENSORS];
} pwr_soc_record_sensor_temp_t;
...
} perf_core_event_amu_counters_t;

typedef struct {
    telemetry_collection_hdr_t collection_header;
    union {
        uint64_t counters[NUMBER_OF_AMU_COUNTERS];
        perf_core_event_amu_counters_t amu_counter_collection;
    };
} perf_core_collection_amu_counters_t;

typedef struct {
    telemetry_record_hdr_t record_header;
    perf_core_collection_amu_counters_t amu_counter_collection[NUMBER_OF_CORES_PER_DIE];
} perf_core_record_amu_counters_t;
