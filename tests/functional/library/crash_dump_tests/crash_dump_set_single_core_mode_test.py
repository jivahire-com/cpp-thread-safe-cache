...
class crash_dump_set_single_core_mode_test(EchoFallsBaseTest):
    ...
    def __init__(
        self,
        name: str = "cd_set_single_core_mode_test",
        number: str = "NaN",
    ...
    def crash_dump_set_single_core_mode_test(self):
        """
        Crash Dump test:
            1. Setup the Test.
            ...
            3. Teardown Test.
        """
        self.log.info("Running Crash Dump set single core mode tests on Die0  . . .")
        self.dut.setup()
        ...
        try:
            ...
            mcp_channel.read_until(key="McpHeartBeat", timeout_seconds=300)
        except Exception as e:
            self.log.error(f"Error Crash Dump set single core mode. Die0: {e}")
            self.test_notify(step="Crash Dump set single core mode.", msg="Test Fail", _is_error=True)
            scp_channel.close()
