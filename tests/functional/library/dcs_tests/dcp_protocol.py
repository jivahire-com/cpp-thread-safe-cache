...
class data_collection_protocol:
    ...
    class client_start_stop_msg:
        ...
        class dcp_msg_start_stop_t(ctypes.LittleEndianStructure):
            ...
            def __init__(self, state: 'data_collection_protocol.client_start_stop_msg.dcp_start_stop_state_t'):
                super().__init__()
                self.state = state
    ...
    class client_get_state_msg:
        ...
        class dcp_msg_get_client_state_t(ctypes.LittleEndianStructure):
            ...
            def __init__(self, state: 'data_collection_protocol.client_get_state_msg.dcp_msg_get_client_state_t'):
                super().__init__()
                self.state = state
    ...
    class client_read_data_msg:
        ...
        class dcp_msg_read_data_t(ctypes.LittleEndianStructure):
            ...
            def __init__(self, physical_start_addr: ctypes.c_uint64, physical_buffer_size: ctypes.c_uint64, rd_data_addr_offset: ctypes.c_uint64, rd_data_size: ctypes.c_uint64, crc: ctypes.c_uint32):
                super().__init__()
    ...
    class client_read_data_complete_msg:
        ...
        class dcp_msg_read_data_complete_t(ctypes.LittleEndianStructure):
            ...
            def __init__(self, rd_data_addr_offset: ctypes.c_uint64, rd_data_size: ctypes.c_uint64):
                super().__init__()
                self.rd_data_addr_offset = rd_data_addr_offset
                self.rd_data_size = rd_data_size
                self.reserved1 = 0
...
data_collection_protocol.client_events_enable_disable_msg.dcp_msg_events_enable_disable_t._fields_ = [
                ...
                ("events", data_collection_protocol.client_events_enable_disable_msg.event * data_collection_protocol.client_events_enable_disable_msg.MAX_EVENTS),  # array of Event structs
            ]

def event_list_init(self, number_of_events: int, events: list[tuple[int, int, int]]):
    if number_of_events > data_collection_protocol.client_events_enable_disable_msg.MAX_EVENTS:
        raise ValueError(f"number_of_events, {number_of_events}, cannot exceed {data_collection_protocol.client_events_enable_disable_msg.MAX_EVENTS}")
    ...
    events = list(events) + [(0, 0, 0)] * data_collection_protocol.client_events_enable_disable_msg.MAX_EVENTS  # Ensure it's a list before concatenation
    events = events[:data_collection_protocol.client_events_enable_disable_msg.MAX_EVENTS]  # Trim to exactly MAX_EVENTS elements

    # Assign the event values to the struct array
    for i in range(data_collection_protocol.client_events_enable_disable_msg.MAX_EVENTS):
        provider_id, event_id, state = events[i]
        self.events[i] = data_collection_protocol.client_events_enable_disable_msg.event(provider_id, event_id, state)


data_collection_protocol.client_events_enable_disable_msg.dcp_msg_events_enable_disable_t.__init__ = event_list_init