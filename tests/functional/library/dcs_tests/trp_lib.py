...
# & (Join-Path ([System.Environment]::GetEnvironmentVariable("REPO_APP_PATH_python.win64", "Process")) "/tools/python.exe") tests/functional/pythia/pylibs/dcs_tests/trp_lib.py
import os
import sys
import time
import ctypes
...
class trp_endpoint(ABC):
    ...
    @abstractmethod
    def send_dcp_message(self, dest_die: ctypes.c_uint8, dest_cpu: transfer_relay_protocol.cpu_type, client_id: data_collection_protocol.dcp_client_id_t, dcp_msg: list[int], timeout_sec: int = None) -> str:
        pass
...
class dcs_cli_trp_endpoint(trp_endpoint):
    ...
    def send_dcp_message(self, dest_die: ctypes.c_uint8, dest_cpu: transfer_relay_protocol.cpu_type, client_id: data_collection_protocol.dcp_client_id_t, dcp_msg: list[int], timeout_sec: int = None) -> str:
        ...
        trp_msg_hdr.source_die_id = self.source_die
        trp_msg_hdr.source_cpu_id = self.source_cpu.value
        trp_msg_hdr.dest_die_id = dest_die
        trp_msg_hdr.dest_cpu_id = dest_cpu.value
        ...
        total_length = ctypes.sizeof(transfer_relay_protocol.trp_msg_hdr_t) + len(dcp_msg)
        send_str = f"dcs trp_send " + struct_to_hex_string(trp_msg_hdr) + byte_list_to_hex_string(dcp_msg)
        ...
        logger.debug(f"TRP Header - Source Die ID: {trp_msg_hdr.source_die_id}")
        logger.debug(f"TRP Header - Source CPU ID: {trp_msg_hdr.source_cpu_id}")
        logger.debug(f"TRP Header - Destination Die ID: {trp_msg_hdr.dest_die_id}")
        logger.debug(f"TRP Header - Destination CPU ID: {trp_msg_hdr.dest_cpu_id}")
