...
class mscp_ddrboot_test(EchoFallsBaseTest):
    ...
    def __init__(
        self,
        name: str = "MSCP_DDRBoot_Die0_Test",
        number: str = "NaN",
    ...
    def mscp_ddrboot_test(self):
        ...
        MSCP ddrboot test:
            ...
            3. Teardown Test.
        """
        self.log.info("Running MSCP ddr boot test . . .")
        ...
        try:
            scp_connection.get_current_channel().read_until(key="BDAT DONE", timeout_seconds=900)
       
            scp_connection.get_current_channel().read_until(key="SMBIOS 16 DONE", timeout_seconds=900)
       
            self.log.info("Reading SCP UART for SMBIOS 17 DONE . . .")
            scp_connection.get_current_channel().read_until(key="SMBIOS 17 DONE", timeout_seconds=900)
            ...
            self.test_notify(step="DDRBoot", msg="Test Fail", _is_error=True)
            self.dut.teardown()
            return False
        finally:
            scp_connection.get_current_channel().close()
