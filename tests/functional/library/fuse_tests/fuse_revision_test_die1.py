...
- Test that checks for Config Manager CLI List Die1 Command.
"""
import time
import sys, os
...
class fuse_revision_test_die1(EchoFallsBaseTest):
    ...
    def __init__(
        self,
        name: str = "fuse_revision_test_die1",
        number: str = "NaN",
    ...
    def fuse_revision_test_die1(self):
        """ 
        Test function: 
            1. Setup the Test. 
            ...
            4. Teardown Test. 
        """
        self.log.info("Running fuse revision command Test. . .")
        self.dut.setup()
        ...
        self.log.info("Submitting fuse revision command . . .")
        command="fuse"
        core_com_channel.write_line(write_string=command)

        for attempt in range(2):
            command="sample_detail"
            self.log.info(f"Submitting sample command command {command} . . .") 
            core_com_channel.write_line(write_string=command)
