...
class mts_read_intercore_block:
    def __init__(self, host: str, port: int, source_die: int, source_cpu: transfer_relay_protocol.cpu_type):
        self.host = host
        self.port = port
    ...
    def open_telnet(self):
        ...
        self.source_comm_channel = Telnet_(host=self.host, port=self.port, encoding="UTF-8")
    ...
    def send_read_intercore_block(self, core_id: int):
        ...
        trp_msg_hdr.mts_client_id = data_collection_protocol.mts_client_id_t.MTS_CLIENT_ID_PWR_INST_TELEM
        trp_msg_hdr.trp_msg_id = transfer_relay_protocol.trp_msg_id_t.TRP_MSG_ID_READ_INTERCORE_BLOCK.value
        trp_msg_hdr.source_seq_num = self.get_next_sequence_number()
        trp_msg_hdr.payload_size = ctypes.sizeof(transfer_relay_protocol.trp_block_read_req_t)
        ...
        # Send the command
        send_str = f"mts trp_send " + struct_to_hex_string(trp_msg_hdr) + struct_to_hex_string(block_read_req)
        logger.debug(f"Sending TRP_MSG_ID_READ_INTERCORE_BLOCK: {send_str}")
        self.source_comm_channel.write_line(write_string=send_str)

        # Read the response
        response = self.source_comm_channel.read_until(key="TrpRx", timeout_seconds=self.default_timeout_sec)
        ...
        # Extract the header
        header = transfer_relay_protocol.trp_msg_hdr_t.from_buffer_copy(response_bytes[:ctypes.sizeof(transfer_relay_protocol.trp_msg_hdr_t)])

        # Validate the header status
        if header.trp_msg_status != transfer_relay_protocol.trp_status_t.TRP_STATUS_SUCCESS:
            raise ValueError(f"Header validation failed with status: {header.trp_msg_status}")

        # Extract the payload
        payload = transfer_relay_protocol.trp_msg_read_block_rsp_t.from_buffer_copy(response_bytes[ctypes.sizeof(transfer_relay_protocol.trp_msg_hdr_t):])
    ...
    def send_read_intercore_block_complete(self, core_id: int):
        ...
        trp_msg_hdr.dest_node.core_id = core_id
        trp_msg_hdr.mts_client_id = data_collection_protocol.mts_client_id_t.MTS_CLIENT_ID_PWR_INST_TELEM
        trp_msg_hdr.trp_msg_id = transfer_relay_protocol.trp_msg_id_t.TRP_MSG_ID_READ_INTERCORE_BLOCK_COMPLETE.value
        trp_msg_hdr.source_seq_num = self.get_next_sequence_number()
        trp_msg_hdr.payload_size = ctypes.sizeof(transfer_relay_protocol.trp_msg_read_block_rsp_t)
...
def main():
    ...
    script = mts_read_intercore_block(host="localhost", port=4256, source_die=0, source_cpu=transfer_relay_protocol.cpu_type.CPU_MCP)
    ...
    try:
        ...
        script.send_read_intercore_block_complete(transfer_relay_protocol.cpu_type.CPU_SDM)
        script.send_read_intercore_block_complete(transfer_relay_protocol.cpu_type.CPU_CDED_SDM)
    finally:
        script.close_telnet()
