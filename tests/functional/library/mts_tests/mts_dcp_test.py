...
class MtsDcpTest(EchoFallsBaseTest):
    ...
    def wait_for_scp_mcp_heartbeat(self) -> bool:
        ...
        try:
            ...
            try:
                self.die0_scp_channel.read_until(
                    key="ScpHeartBeat", timeout_seconds=1800
                )
            ...
            try:
                self.die0_mcp_channel.read_until(
                    key="McpHeartBeat", timeout_seconds=3600
                )
    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            self.die0_scp_trp_endpoint = mts_cli_trp_endpoint(
                self.die0_scp_channel, 0, transfer_relay_protocol.cpu_type.CPU_SCP
            )
    ...
    def test_client_read_data_polling(self, command=None):
        ...
        try:
            ...
            self.log.debug(
                "Provider id 0x202, event id 0x4   (SOC MAX TEMP).  Send to power telemetry client."
            )
            event_tuple = [
                (
                    0x0202,
            ...
            self.log.debug(
                "Send DCP_EVENTS_ENABLE_STATE_ENABLE to enable an instantaneous event"
            )
            ...
            while True:
                ...
                while True:
                    ...
                    self.log.debug(
                        f"Client_read_data status : {status} and client_read_data response : {response}"
                    )
                    if status not in [
                        ...
                        self.log.debug(
                            f"No Package data available for {loop_counter} iteration"
                        )
                        ...
                        self.log.debug(
                            f"Package data available for {loop_counter} iteration, Total package count received till now is {package_count}"
                        )

                        self.log.debug(
                            f"phys: {response.physical_start_addr:x}, size: {response.physical_buffer_size:x}, offset: {response.rd_data_addr_offset:x}, size: {response.rd_data_size:x}, crc: {response.crc:x}\n"
                        )
                        self.log.debug(
                            "Construct read complete message and send back to power telemetry client."
                        )
            ...
            if package_count > 0:
                self.log.info(
                    f"PASS : Test CLIENT_READ_DATA and CLIENT_READ_DATA_COMPLETE ,Total Package data received {package_count}"
                )
                return True
            else:
                self.log.info(
                    f"FAIL : Test CLIENT_READ_DATA and CLIENT_READ_DATA_COMPLETE ,Total Package data received {package_count}"
                )
                return False
    ...
    def test_client_get_manifest(self, command=None):
        ...
        try:
            ...
            self.log.info(
                f"Client_get_manifest status : {status} and client_get_manifest response : {response}"
            )
    ...
    def test_client_get_platform_information(self, command=None):
        ...
        try:
            ...
            self.log.info(
                f"client_get_platform_information status : {status} and client_get_platform_information response : {response}"
            )
            ...
            if response.dcp_ver_major == 1:
                self.log.info(
                    f"PASS :DCP Version Major is as expected: {response.dcp_ver_major}"
                )
            else:
                self.log.error(
                    f"❌ FAIL :DCP Version Major is NOT as expected: {response.dcp_ver_major}"
                )
                return False

            if response.dcp_ver_minor == 0:
                self.log.info(
                    f"PASS :DCP Version Minor is as expected: {response.dcp_ver_minor}"
                )
            else:
                self.log.error(
                    f"❌ FAIL :DCP Version Minor is NOT as expected: {response.dcp_ver_minor}"
                )
                return False

            if (
                response.plat_id
                == data_collection_protocol.dcp_msg_get_plat_info_t.COBALT_200
            ):
                self.log.info(
                    f"PASS : Fetched Platform ID {response.plat_id} is matching with DCP_PLATFORM_COBALT_200 {data_collection_protocol.dcp_msg_get_plat_info_t.COBALT_200}"
                )
            else:
                self.log.info(
                    f"FAIL : Fetched Platform ID {response.plat_id} is NOT matching with DCP_PLATFORM_COBALT_200 {data_collection_protocol.dcp_msg_get_plat_info_t.COBALT_200}"
                )
                return False
    ...
    def test_client_get_capabilities(self, command=None):
        ...
        try:
            ...
            self.log.info(
                f"client_get_capabilities status : {status} and response : {response}"
            )
            self.log.info("Verify the following bitfields are returned per spec")
            if all(
                val == 1
                for val in [
                    response.DCP_MSG_ID_GET_CAPABILITIES,
                    response.DCP_MSG_ID_GET_MANIFEST,
                    response.DCP_MSG_ID_RESET,
                    response.DCP_MSG_ID_GET_PLAT_INFO,
                ]
            ):
            ...
            self.log.info(
                f"DCP_MSG_ID_GET_CAPABILITIES     : {response.DCP_MSG_ID_GET_CAPABILITIES}"
            )
            self.log.info(
                f"DCP_MSG_ID_GET_MANIFEST         : {response.DCP_MSG_ID_GET_MANIFEST}"
            )
            self.log.info(
                f"DCP_MSG_ID_RESET                : {response.DCP_MSG_ID_RESET}"
            )
            self.log.info(
                f"DCP_MSG_ID_GET_PLAT_INFO        : {response.DCP_MSG_ID_GET_PLAT_INFO}"
            )
            ...
            if all(
                ...
                for val in [
                    response.DCP_MSG_ID_GET_CAPABILITIES,
                    response.DCP_MSG_ID_GET_STATE,
                    response.DCP_MSG_ID_EVENTS_ENABLE_DISABLE,
                    response.DCP_MSG_ID_START_STOP,
                    response.DCP_MSG_ID_READ_DATA,
                    response.DCP_MSG_ID_READ_DATA_COMPLETE,
                    response.DCP_MSG_ID_RESET,
                ]
            ):
                self.log.info("PASS : All Below bitfields are set as expected")
            ...
            self.log.info(
                f"DCP_MSG_ID_GET_STATE            : {response.DCP_MSG_ID_GET_STATE}"
            )
            self.log.info(
                f"DCP_MSG_ID_EVENTS_ENABLE_DISABLE: {response.DCP_MSG_ID_EVENTS_ENABLE_DISABLE}"
            )
            self.log.info(
                f"DCP_MSG_ID_START_STOP           : {response.DCP_MSG_ID_START_STOP}"
            )
            self.log.info(
                f"DCP_MSG_ID_READ_DATA            : {response.DCP_MSG_ID_READ_DATA}"
            )
            self.log.info(
                f"DCP_MSG_ID_READ_DATA_COMPLETE   : {response.DCP_MSG_ID_READ_DATA_COMPLETE}"
            )
            self.log.info(
                f"DCP_MSG_ID_RESET                : {response.DCP_MSG_ID_RESET}"
            )
    ...
    def test_client_start_stop(self, command=None):
        ...
        try:
            ...
            if (
                client_state
                != data_collection_protocol.client_get_state_msg.dcp_client_state_t.DCP_CLIENT_STATE_STOPPED
            ):
            ...
            if (
                client_state
                != data_collection_protocol.client_get_state_msg.dcp_client_state_t.DCP_CLIENT_STATE_RUNNING
            ):
...
# & (Join-Path ([System.Environment]::GetEnvironmentVariable("REPO_APP_PATH_python.win64", "Process")) "/tools/python.exe") tests/functional/library/mts_tests/mts_dcp_test.py
#
...
def main():
    ...
    while True:
        ...
        while True:
            ...
            print(
                f"phys: {response.physical_start_addr:x}, size: {response.physical_buffer_size:x}, offset: {response.rd_data_addr_offset:x}, phys offset start: {response.physical_start_addr + response.rd_data_addr_offset:x}, size: {response.rd_data_size:x}, crc: {response.crc:x}\n"
            )
