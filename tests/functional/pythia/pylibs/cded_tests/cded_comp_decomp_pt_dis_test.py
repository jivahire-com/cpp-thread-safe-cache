...
- Test the CDED Compress (verified by Decompress) and Decompress functionality from AP (Core 0) when passthrough is disabled.
"""
...
class cded_comp_decomp_pt_dis_test(EchoFallsBaseTest):
    ...
    def cded_comp_decomp_pt_dis_test(self, pass_logs, fail_logs):
        """ 
        Test function: 
            1. Setup the Test. 
            2. Wait for AP to be up. Send 'ap_bm cded_test -o cx' command 
            3. Read response and check if test passed or failed based on response
            4. Teardown Test. 
        """
        self.log.info("Running CDED Compress (followed by Decompress to verify) test")
