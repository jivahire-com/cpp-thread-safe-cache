...
- Test the CDED Decompress from AP (Core 0).
"""
...
class cded_decomp_test(EchoFallsBaseTest):
    ...
    def cded_decomp_test(self, pass_logs, fail_logs):
        """ 
        Test function: 
            1. Setup the Test. 
            2. Wait for AP to be up. Send 'ap_bm cded_test -o x' command 
            3. Read response and check if test passed or failed based on response
            4. Teardown Test. 
        """
        self.log.info("Running CDED decompress test. . .")
        self.dut.setup()
        ...
        command="ap_bm cded_test -o x"
        self.log.info(f"Submitting {command} command . . .") 
        command_response=core_com_channel.execute_command(command="ap_bm cded_test -o x", command_args= "")
