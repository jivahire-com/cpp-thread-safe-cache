...
- Test that checks for Large FIFO Mailbox SCP CDED SEND RECV Command.
"""
...
class largefifo_mailbox_cli_scp_cded_test_send_recv(EchoFallsBaseTest):
    ...
    def __init__(
        self,
        name: str = "LargeFIFOMBX_SCP_CDED_Send_Recv",
        number: str = "NaN",
    ...
    def largefifo_mailbox_cli_scp_cded_test_send_recv(self):
        """ 
        Test function: 
            1. Setup the Test. 
        ...
        self._submit_command(cded_channel, "scp_recv 2")
        self._submit_command(scp_channel, "icc_largembx")

        try:
            self._send_and_receive_commands(scp_channel, cded_channel)
        except Exception as e:
            scp_channel.close()
            cded_channel.close()
    ...
    def _send_and_receive_commands(self, send_channel, receive_channel):
        ...
        for cmd in commands:
            ...
            try:
                ...
                self.log.info("Request SENT Successfully from SCP to CDED...")
                # Validate Send Received message on CDED
                receive_channel.read_until(key="Recv Complete", timeout_seconds=100)
                self.log.info("Received Response Successfully from SCP to CDED...")
            except Exception as e:
