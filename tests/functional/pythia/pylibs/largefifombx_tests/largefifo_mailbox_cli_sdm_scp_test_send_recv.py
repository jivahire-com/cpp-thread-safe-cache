...
class largefifo_mailbox_cli_sdm_scp_test_send_recv(EchoFallsBaseTest):
    ...
    def __init__(
        self,
        name: str = "LargeFIFOMBX_SDM_SCP_Send_Recv",
        number: str = "NaN",
    ...
    def largefifo_mailbox_cli_sdm_scp_test_send_recv(self):
        """ 
        Test function: 
            1. Setup the Test. 
            2. Wait for SDM to be up. Send 'SEND' and 'RECV' command 
        ...
        self._submit_command(scp_channel, "sdm_recv 2")
        self._submit_command(sdm_channel, "scp_mbx")

        try:
            self._send_and_receive_commands(sdm_channel, scp_channel)
        except Exception as e:
            scp_channel.close()
            sdm_channel.close()
    ...
    def _send_and_receive_commands(self, send_channel, receive_channel):
        ...
        for cmd in commands:
            ...
            try:
                ...
                self.log.info("Request SENT Successfully from SDM to SCP...")
                # Validate Send Received message on SDM
                receive_channel.read_until(key="Recv Complete", timeout_seconds=100)
                self.log.info("Received Response Successfully from SDM to SCP...")
            except Exception as e:
