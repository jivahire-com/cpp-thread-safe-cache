...
class mod_power_pstate_transition(EchoFallsBaseTest):
    ...
    def power_module_pstate_transition_test(self):
        ...
        core_com_channel.write_line(write_string=command)
        try:
            command_response_cli = core_com_channel.read_until(key="Allowed plimit maximum: 31", timeout_seconds=300)
        except Exception as e:
            self.log.error(f"Error reading SCP UART: {e}")
        ...
        core_com_channel.write_line(write_string=command)
        try:
            command_response_cli = core_com_channel.read_until(key="Allwd plimit maximum: 31", timeout_seconds=300)
        except Exception as e:
            self.log.error(f"Error reading SCP UART: {e}")
