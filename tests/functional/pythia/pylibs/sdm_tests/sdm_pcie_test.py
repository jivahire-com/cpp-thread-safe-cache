...
class sdm_pcie_test(EchoFallsBaseTest):
    ...
    def sdm_pcie_test(self, pass_logs, fail_logs):
        ...
        command_response=KngPythiaTestIF.write_to_uart(self=self, connection=self.dut.mb.node_0.soc.primary_die.apns.channel_manager, command="ap_bm pcie_test") 
        
        if (self.dut.get_dut_type() == DeviceType.BIGFPGA):
            test_result = KngPythiaTestIF.parse_log(self=self, log_lines=command_response[1], pass_logs=pass_logs, fail_logs=fail_logs)

        elif (self.dut.get_dut_type() == DeviceType.SVP):
            command_response=KngPythiaTestIF.read_from_uart(self=self, connection=self.dut.mb.node_0.soc.primary_die.apns.channel_manager, num_lines=17)
