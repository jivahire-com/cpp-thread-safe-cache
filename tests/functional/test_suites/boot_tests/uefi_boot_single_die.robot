...
Library     library.boot_tests.uefi_boot
...         workspace_config=${WORKSPACE_CONFIG}
...         default_log_home=${LOG_DIR}
...         fw_payload_path=${PAYLOAD_DIR}
...         host_config=${HOST_CONFIG_DIR}/hsp_scp_bl_embed_fw.json
...         WITH NAME    uefi_boot_test_lib

*** Test Cases ***
Test Case - MSCP: UEFI boot Test
    # Get an instance of the test library
    ${test_lib} =    Get Library Instance    uefi_boot_test_lib
    
    # Call the test method from the test library and store the result
    ${test_result} =    Call Method    ${test_lib}    uefi_boot
