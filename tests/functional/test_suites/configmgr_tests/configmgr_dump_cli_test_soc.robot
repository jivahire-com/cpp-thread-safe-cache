...
*** Settings ***
Documentation    Verifies Config Manager CLI Dump command.

# Import the Python library. Class name must match filename when using file paths.
# Importing also calls __init__().
Library     ${CURDIR}/../../library
Library     library.configmgr_tests.configmgr_dump_cli_test
...
*** Test Cases ***
Test Case - Config Manager CLI Dump Test
    [Documentation]     Verifies Config Manager CLI Dump command.

    # Get an instance of the configmgr_dump_cli_test_lib library
    ${test_lib} =       Get Library Instance      configmgr_dump_cli_test_lib

    # Check for both failure and success strings in the output.
    ${test_result} =        Call Method         ${test_lib}         configmgr_dump_cli_test
