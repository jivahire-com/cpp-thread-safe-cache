...
*** Settings ***
Documentation    Verifies D2D Mailbox CLI send recv command.

# Import the Python library. Class name must match filename when using file paths.
# Importing also calls __init__().
Library     ${CURDIR}/../../library
Library     library.d2dmbx_tests.d2d_mailbox_cli_test_send_recv
...         workspace_config=${WORKSPACE_CONFIG}
...         default_log_home=${LOG_DIR}
...         fw_payload_path=${PAYLOAD_DIR}
...         host_config=${HOST_CONFIG_DIR}/hsp_scp_bl_embed_fw_dual_die.json
...         WITH NAME    d2d_mailbox_cli_test_send_recv_lib

*** Test Cases ***
Test Case - D2D Mailbox CLI Send Recv Test
    [Documentation]     Verifies D2D Mailbox CLI Send Recv command.

    # Get an instance of the d2d_mailbox_cli_test_lib library
    ${test_lib} =       Get Library Instance      d2d_mailbox_cli_test_send_recv_lib

    # Check for both failure and success strings in the output.
    ${test_result} =        Call Method         ${test_lib}         d2d_mailbox_cli_test_send_recv
