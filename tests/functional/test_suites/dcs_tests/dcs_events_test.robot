<<<<<<<< HEAD:tests/functional/test_suites/dcs_tests/dcs_dcp_test.robot
...
*** Settings ***
Documentation    Verifies Data Collection System (DCS) events functionality.
...    
...    This robot test is only verify the DCS library functionality by:
...    1. Testing the DCS client start/stop commands <Will add more protocols>
...    2. Verifying command responses through ICC MHU interface
...    3. Validating proper setup and teardown of test environment

Library    ${CURDIR}/../../library

Library    library.dcs_tests.dcs_events_test.DcsEventsTest
...    workspace_config=${WORKSPACE_CONFIG}
...    default_log_home=${LOG_DIR}
...    fw_payload_path=${PAYLOAD_DIR}
...    host_config=${HOST_CONFIG_DIR}/hsp_scp_bl_embed_fw.json
...    WITH NAME    dcs_events_test_lib
...
*** Keywords ***
Setup Test Environment
    ${test_lib}=    Get Library Instance    dcs_events_test_lib
    ${setup_result}=    Call Method    ${test_lib}    setup
    Should Be True    ${setup_result}    Failed to setup DUT
...
Test Case - DCS Client Start Stop Test
    ...
    ...    
    ...    Test Steps:
    ...    1. Send START command using ICC MHU interface and verify response
    ...    2. Send STOP command using ICC MHU interface and verify response
    [Tags]    ssi    ssi_dcs_event_start_stop    dcs    dcs_client    TEST_CASE_ID:2356228
    
    ${test_lib}=    Get Library Instance    dcs_events_test_lib
    Log To Console    \nExecuting DCS Client Start/Stop Test...
    ${response}=    Call Method    ${test_lib}    test_client_start_stop
    Log To Console    \nTest Result: ${response}
    Should Contain    ${response}    Ok    msg=DCS Client Start/Stop test failed
>>>>>>>> c36f05805 (feat: remove the redundant pythia directory under ./tests/functional/):tests/functional/test_suites/dcs_tests/dcs_events_test.robot
