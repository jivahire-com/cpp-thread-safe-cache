...
*** Settings ***
Documentation    Verifies Large FIFO Mailbox CLI CDED SCP send recv command.

# Import the Python library. Class name must match filename when using file paths.
# Importing also calls __init__().
Library     ${CURDIR}/../../library
Library     library.largefifombx_tests.largefifo_mailbox_cli_cded_scp_test_send_recv
...         workspace_config=${WORKSPACE_CONFIG}
...         default_log_home=${LOG_DIR}
...         fw_payload_path=${PAYLOAD_DIR}
...         host_config=${HOST_CONFIG_DIR}/mscp_runtime_fw.json
...         WITH NAME    largefifo_mailbox_cli_cded_scp_test_send_recv_lib
...
Test Case - Large FIFO Mailbox CLI Send Recv Test
    ...
    ${test_lib} =       Get Library Instance      largefifo_mailbox_cli_cded_scp_test_send_recv_lib

    # Check for both failure and success strings in the output.
    ${test_result} =        Call Method         ${test_lib}         largefifo_mailbox_cli_cded_scp_test_send_recv
