...
extern const uint32_t SCP_DTCM_RAM_SIZE = (512 * 1024);

uint8_t test_itc_ram[SCP_TOP_SCP_INST_RAM_SIZE];
uint8_t test_dtc_ram[SCP_DTCM_RAM_SIZE];

static uint8_t cmp_itc_ram[SCP_TOP_SCP_INST_RAM_SIZE] = {0};
static uint8_t cmp_dtc_ram[SCP_DTCM_RAM_SIZE] = {0};
...
TEST_FUNCTION(test_arch_exception_reset_cold_boot, nullptr, nullptr)
{
    ...
    memset(test_dtc_ram, 0xCD, SCP_DTCM_RAM_SIZE);

    memset(cmp_itc_ram, 0x0, SCP_TOP_SCP_INST_RAM_SIZE);
    memset(cmp_dtc_ram, 0x0, SCP_DTCM_RAM_SIZE);
    ...
    assert_true(memcmp(test_dtc_ram, cmp_dtc_ram, SCP_DTCM_RAM_SIZE) == 0);
}
...
TEST_FUNCTION(test_arch_exception_reset_warm_boot, nullptr, nullptr)
{
    ...
    memset(cmp_itc_ram, 0x0, SCP_TOP_SCP_INST_RAM_SIZE);
    memset(cmp_dtc_ram, 0xCD, SCP_DTCM_RAM_SIZE);
