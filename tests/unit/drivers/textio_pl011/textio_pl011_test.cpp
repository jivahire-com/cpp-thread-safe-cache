...
static FPFW_TEXT_IO_ASYNC_READ_REQUEST s_async_read_req = {};
static FPFW_TEXT_IO_ASYNC_WRITE_REQUEST s_async_write_req = {};
...
TEST_FUNCTION(textio_pl011_device_interface_read_sync, test_setup, NULL)
{
    ...
    int32_t status = FpFwTextIoTryReadRequestSync(&s_test_interface.header, s_test_buffer, BUFFER_SIZE_BYTES, &read_bytes);
...
TEST_FUNCTION(textio_pl011_device_interface_write_sync, test_setup, NULL)
{
    ...
    // clang-format off
    int32_t status = FpFwTextIoTryWriteRequestSync(
...
{
    DfwkAsyncRequestInititalize(&s_async_read_req.Header, sizeof(FPFW_TEXT_IO_ASYNC_READ_REQUEST));
    ...
    FpFwTextIoReadRequestAsync(&s_test_interface.header, &s_async_read_req.Header, s_test_buffer, BUFFER_SIZE_BYTES, async_cb, nullptr);

    assert_int_equal(s_async_read_req.Output.ReadBytes, BUFFER_SIZE_BYTES);
}
...
TEST_FUNCTION(textio_pl011_device_interface_read_async_timer, test_setup, NULL)
{
    ...
    assert_true(s_async_read_req.Output.ReadBytes < BUFFER_SIZE_BYTES);
...
{
    DfwkAsyncRequestInititalize(&s_async_write_req.Header, sizeof(FPFW_TEXT_IO_ASYNC_WRITE_REQUEST));
    ...
    FpFwTextIoWriteRequestAsync(&s_test_interface.header, &s_async_write_req.Header, s_test_buffer, BUFFER_SIZE_BYTES, async_cb, nullptr);

    assert_int_equal(s_async_write_req.Output.WrittenBytes, BUFFER_SIZE_BYTES);
}

TEST_FUNCTION(textio_pl011_device_interface_write_async_timer, test_setup, NULL)
{
    DfwkAsyncRequestInititalize(&s_async_write_req.Header, sizeof(FPFW_TEXT_IO_ASYNC_READ_REQUEST));
    ...
    assert_true(s_async_write_req.Output.WrittenBytes < BUFFER_SIZE_BYTES);
