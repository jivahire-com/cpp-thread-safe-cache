...
// External variables needed for proper test setup
...
static int32_t test_setup(void** state)
{
    ...
    // Initialize in_band_publishing_active like other tests
...
{
    FPFW_UNUSED(state);
    reset_pwr_tlm_data(); // Clean up telemetry state like other tests
    return 0;
}
...
static void setup_d2d_pmu_mocks_for_interface(uint8_t interface_id,
                                              ...
                                              uint64_t l1_rx_res)
{
    // Validate interface_id to prevent array overruns
    if (interface_id >= NUMBER_OF_D2D_INTERFACES)
    {
        fail_msg("Invalid interface_id in mock setup");
        return;
    }
...
TEST_FUNCTION(test_d2d_link_state_basic_flow, test_setup, test_teardown)
{
    ...
    // Validate NUMBER_OF_D2D_INTERFACES is reasonable to prevent loop issues
    assert_true(NUMBER_OF_D2D_INTERFACES > 0 && NUMBER_OF_D2D_INTERFACES <= 8);
    ...
    // Step 4: Create D2D link state record and verify contents
    ...
    assert_true(record_size <= sizeof(d2d_record));
    ...
    // Validate record structure integrity before accessing arrays
    // Check that the record header indicates valid collections
    assert_true(d2d_record.record_header.number_of_collections > 0);
    assert_true(d2d_record.record_header.number_of_collections <= NUMBER_OF_D2D_INTERFACES);

    // Validate that we have valid link state data in the first collection
    assert_true(d2d_record.d2d_link_collection[0].collection_header.number_of_elements >= D2D_LINK_MAX);

    // Additional bounds checking for enum values
    assert_true(D2D_LINK_L0 < NUMBER_OF_D2D_LINKS_STATE);
    assert_true(D2D_LINK_L0S < NUMBER_OF_D2D_LINKS_STATE);
    assert_true(D2D_LINK_L1 < NUMBER_OF_D2D_LINKS_STATE);

    // --- Extract actual values with defensive programming ---
    // Validate record structure before accessing array elements
    if (record_size > sizeof(telemetry_record_hdr_t) && d2d_record.record_header.number_of_collections > 0)
    {
        ...
        if (g_print_debug)
        {
            printf("\u2713 All D2D link state verifications passed\n");
        }
    }
    else
    {
        printf("ERROR: D2D record validation failed - record_size=%u, collections=%u\n",
               record_size,
               d2d_record.record_header.number_of_collections);
        fail_msg("D2D link collection is empty or invalid");
    }

    if (g_print_debug)
    {
        printf("=== D2D LINK STATE TEST COMPLETE ===\n");
    }
}