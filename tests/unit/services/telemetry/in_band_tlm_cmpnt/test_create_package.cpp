...
TEST_FUNCTION(test_get_pwr_core_pstate_data, test_setup, test_teardown)
{
    ...
    {
        assert_int_equal(record.pstate_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
        ...
        assert_memset_to_ff((uint8_t *)record.pstate_collection[core_id].pstate_element, sizeof(pwr_core_element_pstate_t) * NUMBER_OF_PSTATES);
    }
}
...
TEST_FUNCTION(test_get_pwr_core_cstate_data, test_setup, test_teardown)
{
    ...
    for (uint16_t core_id = 0; core_id < NUMBER_OF_CORES_PER_DIE; core_id++)
    {
        assert_int_equal(record.cstate_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
        ...
        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)record.cstate_collection[core_id].cstate_element, sizeof(pwr_core_element_cstate_t) * NUMBER_OF_CSTATES);
    }
}
...
TEST_FUNCTION(test_get_pwr_core_throttle_data, test_setup, test_teardown)
{
    ...
    for (uint16_t core_id = 0; core_id < NUMBER_OF_CORES_PER_DIE; core_id++)
    {
        assert_int_equal(record.throttle_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
        assert_int_equal(record.throttle_collection[core_id].collection_header.element_id, POWER_TELEMETRY_ELEMENT_CORE_THROTTLE);
        ...
        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)record.throttle_collection[core_id].throttle_element, sizeof(pwr_core_element_throttle_t) * NUMBER_OF_THROTTLE_TYPES);
    }
}
...
TEST_FUNCTION(test_get_pwr_core_rack_priority_data, test_setup, test_teardown)
{
    ...
    for (uint16_t core_id = 0; core_id < NUMBER_OF_CORES_PER_DIE; core_id++)
    {
        assert_int_equal(record.rack_priority_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
        assert_int_equal(record.rack_priority_collection[core_id].collection_header.element_id, POWER_TELEMETRY_ELEMENT_CORE_RACK_PRIORITIES);
        assert_int_equal(record.rack_priority_collection[core_id].collection_header.collection_id, core_id);
        assert_int_equal(record.rack_priority_collection[core_id].collection_header.number_of_elements, NUMBER_OF_RACK_PRIORITIES);
        ...
        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)record.rack_priority_collection[core_id].rack_priority_element, sizeof(pwr_core_element_rack_priorities_t) * NUMBER_OF_RACK_PRIORITIES);
    }
}
...
TEST_FUNCTION(test_get_pwr_core_voltage_data, test_setup, test_teardown)
{
    ...
    for (uint16_t core_id = 0; core_id < NUMBER_OF_CORES_PER_DIE; core_id++)
    {
        assert_int_equal(record.voltage_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
        ...
        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)&record.voltage_collection[core_id].voltage_element, sizeof(pwr_core_element_voltage_t));
    }
}
...
TEST_FUNCTION(test_get_pwr_core_current_data, test_setup, test_teardown)
{
    ...
    for (uint16_t core_id = 0; core_id < NUMBER_OF_CORES_PER_DIE; core_id++)
    {
        assert_int_equal(record.current_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
        ...
        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)&record.current_collection[core_id].current_element, sizeof(pwr_core_element_current_t));
    }
}
...
TEST_FUNCTION(test_get_pwr_core_temperature_data, test_setup, test_teardown)
{
    ...
    for (uint16_t core_id = 0; core_id < NUMBER_OF_CORES_PER_DIE; core_id++)
    {
        assert_int_equal(record.temperature_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
        assert_int_equal(record.temperature_collection[core_id].collection_header.element_id, POWER_TELEMETRY_ELEMENT_CORE_TEMPERATURE);
        ...
        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)&record.temperature_collection[core_id].temperature_element, sizeof(pwr_core_element_temperature_t));
    }
}
...
TEST_FUNCTION(test_get_pwr_core_histogram_data, test_setup, test_teardown)
{
    ...
    for (uint16_t core_id = 0; core_id < NUMBER_OF_CORES_PER_DIE; core_id++)
    {
        assert_int_equal(record.histogram_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
        assert_int_equal(record.histogram_collection[core_id].collection_header.element_id, POWER_TELEMETRY_ELEMENT_CORE_HISTOGRAM);
        assert_int_equal(record.histogram_collection[core_id].collection_header.collection_id, core_id);
        assert_int_equal(record.histogram_collection[core_id].collection_header.number_of_elements, NUMBER_OF_HS_VOLTAGE_SCALES * NUMBER_OF_HS_TEMP_SCALES);
        ...
        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)record.histogram_collection[core_id].histogram_element, sizeof(pwr_core_element_histogram_t) * NUMBER_OF_HS_VOLTAGE_SCALES * NUMBER_OF_HS_TEMP_SCALES);
    }
}
...
TEST_FUNCTION(test_get_pwr_soc_vr_rail_data, test_setup, test_teardown)
{
    ...
    {
        assert_int_equal(record.rail_collection[rail_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
...
TEST_FUNCTION(test_get_pwr_soc_hnf_data, test_setup, test_teardown)
{
    ...
    {
        assert_int_equal(record.hnf_collection[hnf_chan].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
...
TEST_FUNCTION(test_get_pwr_soc_dimm_data, test_setup, test_teardown)
{
    ...
    {
        assert_int_equal(record.dimm_collection[dimm_module].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
...
TEST_FUNCTION(test_get_pwr_soc_sensor_temp_data, test_setup, test_teardown)
{
    ...
    {
        assert_int_equal(record.sensor_temp_collection[sensor_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
        assert_int_equal(record.sensor_temp_collection[sensor_id].collection_header.element_id, POWER_TELEMETRY_ELEMENT_SOC_SENSOR_TEMP);
        ...
        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)&record.sensor_temp_collection[sensor_id].sensor_temp_element, sizeof(pwr_soc_element_sensor_temp_t));
    }
}
...
TEST_FUNCTION(test_get_pwr_mpam_pstate_data, test_setup, test_teardown)
{
    ...
    {
        assert_int_equal(record.mpam_pstate_collection[mpam_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
        ...
        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)&record.mpam_pstate_collection[mpam_id].mpam_pstate_element, sizeof(pwr_element_mpam_pstate_t) * NUMBER_OF_PSTATES);
    }
}
...
TEST_FUNCTION(test_get_pwr_mpam_throttle_data, test_setup, test_teardown)
{
    ...
    for (uint16_t mpam_id = 0; mpam_id < NUMBER_OF_MPAMS; mpam_id++)
    {
        assert_int_equal(record.mpam_throttle_collection[mpam_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_POWER_TLM_SCHEMA);
        assert_int_equal(record.mpam_throttle_collection[mpam_id].collection_header.element_id, POWER_TELEMETRY_ELEMENT_MPAM_THROTTLE);
        ...
        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)&record.mpam_throttle_collection[mpam_id].mpam_throttle_element, sizeof(pwr_element_mpam_throttle_t));
    }
}
...
TEST_FUNCTION(test_get_inst_core_summary_data, test_setup, test_teardown)
{
    ...
    assert_int_not_equal(record.record_header.record_number, 0);
    assert_int_equal(record.record_header.number_of_collections, NUMBER_OF_CORES_PER_DIE);
    assert_int_equal(record.record_header.record_payload_size, (sizeof(inst_core_record_summary_t) - sizeof(telemetry_record_hdr_t)));

    for (uint16_t core_id = 0; core_id < NUMBER_OF_CORES_PER_DIE; core_id++)
    {
        assert_int_equal(record.inst_core_summary_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_INST_TLM_SCHEMA);
        ...
        assert_int_equal(record.inst_core_summary_collection[core_id].collection_header.collection_payload_size, sizeof(inst_core_collection_summary_t) - sizeof(telemetry_collection_hdr_t));

        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)&record.inst_core_summary_collection[core_id].summary_element, sizeof(inst_core_element_summary_t));
    }
}
...
TEST_FUNCTION(test_get_inst_soc_rail_data, test_setup, test_teardown)
{
    ...
    assert_int_not_equal(record.record_header.record_number, 0);
    assert_int_equal(record.record_header.number_of_collections, MAX_NUM_OF_VR_RAILS);
    assert_int_equal(record.record_header.record_payload_size, (sizeof(inst_soc_record_rail_t) - sizeof(telemetry_record_hdr_t)));

    for (uint16_t rail_id = 0; rail_id < MAX_NUM_OF_VR_RAILS; rail_id++)
    {
        assert_int_equal(record.rail_collection[rail_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_INST_TLM_SCHEMA);
        assert_int_equal(record.rail_collection[rail_id].collection_header.element_id, INST_TELEMETRY_ELEMENT_SOC_RAILS);
        assert_int_equal(record.rail_collection[rail_id].collection_header.collection_id, rail_id);
        assert_int_equal(record.rail_collection[rail_id].collection_header.number_of_elements, 1);
        assert_int_equal(record.rail_collection[rail_id].collection_header.collection_payload_size, sizeof(inst_soc_collection_rail_t) - sizeof(telemetry_collection_hdr_t));
...
TEST_FUNCTION(test_get_inst_soc_dimm_runtime_data, test_setup, test_teardown)
{
    ...
    assert_int_not_equal(record.record_header.record_number, 0);
    assert_int_equal(record.record_header.number_of_collections, NUMBER_OF_DIMM_MODULES);
    assert_int_equal(record.record_header.record_payload_size, (sizeof(inst_soc_record_dimm_runtime_t) - sizeof(telemetry_record_hdr_t)));

    for (uint16_t module_id = 0; module_id < NUMBER_OF_DIMM_MODULES; module_id++)
    {
        assert_int_equal(record.dimm_collection[module_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_INST_TLM_SCHEMA);
        ...
        assert_int_equal(record.dimm_collection[module_id].collection_header.collection_payload_size, sizeof(inst_soc_collection_dimm_runtime_t) - sizeof(telemetry_collection_hdr_t));

        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)&record.dimm_collection[module_id].dimm_rt_element, sizeof(inst_soc_element_dimm_runtime_t));
    }
}
...
TEST_FUNCTION(test_get_inst_soc_dimm_config_data, test_setup, test_teardown)
{
    ...
    assert_int_not_equal(record.record_header.record_number, 0);
    assert_int_equal(record.record_header.number_of_collections, 1);
    assert_int_equal(record.record_header.record_payload_size, (sizeof(inst_soc_record_dimm_config_t) - sizeof(telemetry_record_hdr_t)));
    ...
    assert_int_equal(record.dimm_config_collection.collection_header.collection_payload_size, sizeof(inst_soc_collection_dimm_config_t) - sizeof(telemetry_collection_hdr_t));

    // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
    // This verifies that the correct data ranges are passed to the data processing component get data api's
    assert_memset_to_ff((uint8_t *)&record.dimm_config_collection.dimm_config_element, sizeof(inst_soc_element_dimm_config_t));
}
...
TEST_FUNCTION(test_get_inst_soc_sensor_temp_data, test_setup, test_teardown)
{
    ...
    assert_int_not_equal(record.record_header.record_number, 0);
    assert_int_equal(record.record_header.number_of_collections, NUMBER_OF_SOC_TEMP_SENSORS);
    assert_int_equal(record.record_header.record_payload_size, (sizeof(inst_soc_record_sensor_temp_t) - sizeof(telemetry_record_hdr_t)));

    for (uint16_t sensor_id = 0; sensor_id < NUMBER_OF_SOC_TEMP_SENSORS; sensor_id++)
    {
        assert_int_equal(record.temperature_collection[sensor_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_INST_TLM_SCHEMA);
        assert_int_equal(record.temperature_collection[sensor_id].collection_header.element_id, INST_TELEMETRY_ELEMENT_SOC_TEMP_SENSOR);
        ...
        assert_int_equal(record.temperature_collection[sensor_id].collection_header.collection_payload_size, sizeof(inst_soc_collection_sensor_temp_t) - sizeof(telemetry_collection_hdr_t));

        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)&record.temperature_collection[sensor_id].temperature_element, sizeof(inst_soc_element_sensor_temp_t));
    }
}
...
TEST_FUNCTION(test_get_inst_core_amu_counters_data, test_setup, test_teardown)
{
    ...
    assert_int_not_equal(record.record_header.record_number, 0);
    assert_int_equal(record.record_header.number_of_collections, NUMBER_OF_CORES_PER_DIE);
    assert_int_equal(record.record_header.record_payload_size, (sizeof(inst_core_record_amu_counters_t) - sizeof(telemetry_record_hdr_t)));

    for (uint16_t core_id = 0; core_id < NUMBER_OF_CORES_PER_DIE; core_id++)
    {
        assert_int_equal(record.amu_counter_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_INST_TLM_SCHEMA);
        ...
        assert_int_equal(record.amu_counter_collection[core_id].collection_header.collection_payload_size, sizeof(inst_core_collection_amu_counters_t) - sizeof(telemetry_collection_hdr_t));

        // event data ranges are initialized to 0, the mock Get Api sets them to 0xFF
        // This verifies that the correct data ranges are passed to the data processing component get data api's
        assert_memset_to_ff((uint8_t *)&record.amu_counter_collection[core_id].amu_counter_element, sizeof(inst_core_element_amu_counters_t));
    }
}
...
TEST_FUNCTION(test_package_create_append_to_inst_pkg_some_enabled, test_setup, test_teardown)
{
    ...
    assert_int_equal(package->rail_record.record_header.record_payload_size, (sizeof(inst_soc_record_rail_t) - sizeof(telemetry_record_hdr_t)));

    assert_int_equal(package->sensor_temp_record.record_header.number_of_collections, NUMBER_OF_SOC_TEMP_SENSORS);
    assert_int_equal(package->sensor_temp_record.record_header.record_payload_size, (sizeof(inst_soc_record_sensor_temp_t) - sizeof(telemetry_record_hdr_t)));
...
TEST_FUNCTION(test_package_create_append_to_inst_pkg_too_small, test_setup, test_teardown)
{
    ...
    uint32_t pkg_size = package_create_append_to_inst_pkg((uintptr_t)&cr_max_package_mem, sizeof(inst_report_partial_package_t), &pkg_header);
    assert_int_equal(pkg_size, 0);
}