...
TEST_FUNCTION(test_get_inst_core_summary_data, test_setup, test_teardown)
{
    ...
    create_inst_core_summary_record(&record);
    ...
    for (uint16_t core_id = 0; core_id < NUMBER_OF_CORES_PER_DIE; core_id++)
    {
        assert_int_equal(record.inst_core_summary_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_INST_TLM);
...
TEST_FUNCTION(test_get_inst_soc_rail_data, test_setup, test_teardown)
{
    ...
    create_inst_soc_rail_record(&record);
    ...
    for (uint16_t rail_id = 0; rail_id < MAX_NUM_OF_VR_RAILS; rail_id++)
    {
        assert_int_equal(record.rail_collection[rail_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_INST_TLM);
...
TEST_FUNCTION(test_get_inst_soc_dimm_runtime_data, test_setup, test_teardown)
{
    ...
    create_inst_soc_dimm_runtime_record(&record);
    ...
    {
        assert_int_equal(record.dimm_collection[module_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_INST_TLM);
...
TEST_FUNCTION(test_get_inst_soc_dimm_config_data, test_setup, test_teardown)
{
    ...
    create_inst_soc_dimm_config_record(&record);
    ...
    assert_int_equal(record.dimm_config_collection.collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_INST_TLM);
...
TEST_FUNCTION(test_get_inst_soc_sensor_temp_data, test_setup, test_teardown)
{
    ...
    create_inst_soc_sensor_temp_record(&record);
    ...
    for (uint16_t sensor_id = 0; sensor_id < NUMBER_OF_SOC_TEMP_SENSORS; sensor_id++)
    {
        assert_int_equal(record.temperature_collection[sensor_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_INST_TLM);
...
TEST_FUNCTION(test_get_inst_core_amu_counters_data, test_setup, test_teardown)
{
    ...
    create_inst_core_amu_counters_record(&record);
    ...
    for (uint16_t core_id = 0; core_id < NUMBER_OF_CORES_PER_DIE; core_id++)
    {
        assert_int_equal(record.amu_counter_collection[core_id].collection_header.provider_id,  EVENT_TRACE_PROVIDER_ID_MCP_INST_TLM);
...
TEST_FUNCTION(test_package_create_power_report_some_enabled, test_setup, test_teardown)
{
    ...
    power_report_event_enable[POWER_TELEMETRY_ELEMENT_CORE_PSTATE] = true;
    power_report_event_enable[POWER_TELEMETRY_ELEMENT_SOC_PC3] = true;
...
TEST_FUNCTION(test_package_create_power_report_too_small, test_setup, test_teardown)
{
    ...
    power_report_event_enable[POWER_TELEMETRY_ELEMENT_CORE_PSTATE] = true;
    power_report_event_enable[POWER_TELEMETRY_ELEMENT_SOC_PC3] = true;
    power_report_event_enable[POWER_TELEMETRY_ELEMENT_SOC_VR_RAILS] = true;
...
TEST_FUNCTION(test_package_create_inst_report_none_enabled, test_setup, test_teardown)
{
    inst_report_full_package_t package = {{{0}}};
    ...
    fpfw_status_t status = package_create_inst_report((uintptr_t)&package, sizeof(inst_report_full_package_t));

    assert_int_equal(status, FPFW_STATUS_SUCCESS);
}

TEST_FUNCTION(test_package_create_inst_report_all_enabled, test_setup, test_teardown)
{
    inst_report_full_package_t package = {{{0}}};

    for(uint16_t i = 0; i < INST_TELEMETRY_ELEMENT_ID_MAX; i++)
    {
        inst_report_event_enable[i] = true;
    }
    expect_function_calls(data_proc_tlm_cmpnt_get_inst_soc_core_summary_data, NUMBER_OF_CORES_PER_DIE);
    expect_function_calls(data_proc_tlm_cmpnt_get_inst_soc_rail_data, MAX_NUM_OF_VR_RAILS);
    expect_function_calls(data_proc_tlm_cmpnt_get_inst_soc_dimm_runtime_data, NUMBER_OF_DIMM_MODULES);
...
TEST_FUNCTION(test_package_create_inst_report_some_enabled, test_setup, test_teardown)
{
    typedef struct
    {
        telemetry_package_hdr_t package_header;
    ...
    inst_report_partial_package_t package = {{{0}}};
    ...
    inst_report_event_enable[INST_TELEMETRY_ELEMENT_SOC_RAILS] = true;
    inst_report_event_enable[INST_TELEMETRY_ELEMENT_SOC_TEMP_SENSOR] = true;

    expect_function_calls(data_proc_tlm_cmpnt_get_inst_soc_rail_data, MAX_NUM_OF_VR_RAILS);
    expect_function_calls(data_proc_tlm_cmpnt_get_inst_soc_snsr_temp_data, NUMBER_OF_SOC_TEMP_SENSORS);

    fpfw_status_t status = package_create_inst_report((uintptr_t)&package, sizeof(inst_report_partial_package_t));
...
TEST_FUNCTION(test_package_create_inst_report_too_small, test_setup, test_teardown)
{
      typedef struct
    {
        telemetry_package_hdr_t package_header;
    ...
    inst_report_event_enable[INST_TELEMETRY_ELEMENT_SOC_RAILS] = true;
    inst_report_event_enable[INST_TELEMETRY_ELEMENT_SOC_TEMP_SENSOR] = true;
    inst_report_event_enable[INST_TELEMETRY_ELEMENT_AMU] = true;
