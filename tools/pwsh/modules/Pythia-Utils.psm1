...
.EXAMPLE
Invoke-Pythia-Stress -test .\tests\functional\pythia\test_suites\heart_beat\
#>
Function Invoke-Pythia-Stress(
    [Parameter(Mandatory=$true)] [string] $test,
    [Parameter(Mandatory=$true)] [ValidateSet('svp', 'fpga')] [string] $platform = "svp",
    [Parameter(Mandatory=$true)] [int] $stress_iteration
)
{

    Write-Host ""
    Write-Host "`tInvoking Pythia Functional Test with iteration $stress_iteration"
    Write-Host ""
    ...
    # Loop through the stress iterations
    for ($i = 1; $i -le $stress_iteration; $i++) 
    {
        Write-Host ""
        Write-Host "`tStarting iteration $i of $stress_iteration"
        Write-Host ""        

    # Create test run paths based on timestamp
    $time_stamp = (Get-Date -Format "yyyy_MM_dd_hhmmss")
    $pythia_test_dir = Join-Path -Path $env:REPO_APP_TEST_LOG_DIR -ChildPath "pythia"
    $test_results_dir = Join-Path -Path $pythia_test_dir -ChildPath "$time_stamp-iteration-$i"
...
Write-Host ""
Write-Host "`tAll $stress_iteration completed successfully."
Write-Host ""
}
